...
import { Calendar, Loader2, DollarSign, TrendingUp } from 'lucide-react';
...
export default function DailyCommissionReport() {
  ...
  const fetchCommissionReport = async () => {
    ...
    try {
      ...
      const res = await api.get<CommissionRow[]>(`/reports/commission?startDate=${start}&endDate=${end}`);
      const data = res.data || [];
      ...
      const totalTreatments = data.reduce((sum, row) => sum + Number(row.treatmentCount || 0), 0);
      
      setSummary({ totalRevenue, totalCommission, totalTreatments });
    } catch (e: any) {
      toast.error(e.response?.data?.message || 'Failed to fetch commission report');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-4 md:p-6 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Therapist commission breakdown and analysis</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500">Loading commission report...</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          {/* Commission Breakdown by Therapist */}
          <div className="card">
              ...
              <table className="table text-sm">
                <thead>
                  <tr className="text-left text-gray-600 uppercase text-xs">
                    ...
                    <th className="px-4 py-3 text-right">Avg. Commission</th>
                  </tr>
                </thead>
                <tbody>
                  {commissionData.length === 0 ? (
                    ...
                    commissionData.map((row) => {
                      const avgCommission = row.treatmentCount > 0 ? row.commission / row.treatmentCount : 0;
                      return (
                        <tr key={row.therapistId} className="border-b hover:bg-gray-50">
                          <td className="px-4 py-3 font-medium">{row.therapistName || 'Unknown'}</td>
                          ...
                          <td className="px-4 py-3 text-right text-gray-600">{fmt(avgCommission)}</td>
                        </tr>
                      );
                    })
                  )}
