...
async function runSafeAdditiveMigrations() {
  ...
  `);
    ...
    await db.execute(sql`ALTER TABLE inventory ADD COLUMN IF NOT EXISTS item_type VARCHAR(50) NOT NULL DEFAULT 'OTHER'`);
    await db.execute(sql`ALTER TABLE inventory ADD COLUMN IF NOT EXISTS is_reusable BOOLEAN NOT NULL DEFAULT false`);
    await db.execute(sql`ALTER TABLE inventory ADD COLUMN IF NOT EXISTS reusable_available NUMERIC(14,3) NOT NULL DEFAULT 0`);
    await db.execute(sql`ALTER TABLE inventory ADD COLUMN IF NOT EXISTS reusable_in_use NUMERIC(14,3) NOT NULL DEFAULT 0`);

    // Expand the inventory_movements type check (idempotent, non-destructive).
    await db.execute(sql`
    ...
    await db.execute(sql`
      ...
      CHECK (type IN ('IN','OUT','ADJUSTMENT','RECIPE_CONSUMPTION','OPENING','OPNAME','REVERSAL','PURCHASE','RETAIL_SALE','WASTE','RETURN','TRANSFER'))
    `);

    // Inventory is per-outlet: the same SKU may exist at multiple outlets, each
    // with its own balance + ledger. Replace the legacy global UNIQUE(sku)
    // (constraint name auto-generated by Postgres) with UNIQUE(sku, outlet_id).
    await db.execute(sql`
    ...
    await db.execute(sql`
      DO $$
      BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'inventory_sku_outlet_unique') THEN
          ALTER TABLE inventory ADD CONSTRAINT inventory_sku_outlet_unique UNIQUE (sku, outlet_id);
        END IF;
      END $$;
    `);

    // DB-level idempotency for treatment completion: a booking may produce at
    // most ONE treatment_transaction (walk-in inputs have NULL booking_id and
    // are excluded). This makes double-completion races fail at the database.
    await db.execute(sql`
      CREATE UNIQUE INDEX IF NOT EXISTS treatment_transactions_booking_unique
        ON treatment_transactions(booking_id)
        WHERE booking_id IS NOT NULL
    `);

    // ---- Inventory reconciliation queue (failed recipe consumption) ----
    // Records a failed consumeRecipeForTreatment so admin can see + retry.
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS inventory_reconciliations (
        ...
        status VARCHAR(50) NOT NULL DEFAULT 'PENDING_RECONCILIATION'
          CHECK (status IN ('PENDING_RECONCILIATION','RESOLVED')),
        created_at TIMESTAMP DEFAULT NOW(),
        resolved_at TIMESTAMP,
        resolved_by VARCHAR(255) REFERENCES users(id)
      )
    `);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS inventory_reconciliations_outlet_id_idx ON inventory_reconciliations(outlet_id)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS inventory_reconciliations_status_idx ON inventory_reconciliations(status)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS inventory_reconciliations_reference_id_idx ON inventory_reconciliations(reference_id)`);
